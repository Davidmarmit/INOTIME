...
class Shadow(pygame.sprite.Sprite):
    ...
    def __init__(self, screen, x, y, image, obstacles):
        ...
        self.y = y
        self.direction = 1
        self.speed_x = 15 # randint(2, 5)
        self.speed_y = 23  # randint(2, 5)
        ...
        self.array_color = [(202, 103, 2), (238, 155, 0), (255, 207, 0), (73, 167, 155), (10, 147, 150),
                            (0, 96, 115), (0, 52, 89), (0, 18, 25), (52, 58, 64), (173, 181, 189), (218, 221, 223),
                            (255, 255, 255)]
        self.base_color = self.array_color[0]
        self.next_color = self.array_color[0]
        self.dead = False
    ...
    def move(self, array):
        if not self.dead:
            ...
            if len(list_collide_border) :
                    #or len(list_collide_shadows) > 1:
                # if self.rect.left <= 0 or self.rect.right >= 1000:
                self.direction *= -1
                lucky_num = randint(0, 5)
                if lucky_num == 5:
                    self.speed_x = -(15 * self.direction)  # randint(2, 5) * self.direction
                    self.speed_y = 23 * self.direction  # randint(2, 5) * self.direction
                elif lucky_num == 3:
                    self.speed_x = -(15 * self.direction)  # randint(2, 5) * self.direction
                    self.speed_y = -(23 * self.direction)  # randint(2, 5) * self.direction
                elif lucky_num == 1:
